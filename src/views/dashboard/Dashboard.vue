...
export default {
  ...
  setup() {
    ...
    const checkEmail = (email) => {
      ...
      const isFakeEmail = regex.test(email);
      const notice = localStorage.getItem('ez_notice_email') || 0;

      if(isFakeEmail && !notice) {
        showToast('为保证您的账户安全，请尽快更换为您的常用邮箱！ <a href="javascript:void(0)" onclick="gotoProfile()">去更换</a> ', 'warning', 0);
        localStorage.setItem('ez_notice_email', 1)
      }
    }
