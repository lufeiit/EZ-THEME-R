<template>
    ...
    <div class="profile-inner">
      ...
      <div v-else class="profile-content">
              ...
              <div class="info-item">
                ...
                <span class="info-value">
                  ...
                  <span
                    v-if="isTgEmail"
                    class="change-email"
                    @click="showEmaildModal = true"
                    >{{ $t("profile.changeEmail") }}</span
                  >
                </span>
              </div>
        ...
        <div class="profile-card">
          ...
          <div class="settings-content">
            <div class="setting-item">
              <div class="setting-info">
                <span class="setting-label">{{
                  $t("profile.expireRemind")
                }}</span>

                <span class="setting-description">{{
                  $t("profile.expireRemindDesc")
                }}</span>
              </div>
              ...
              <div class="setting-toggle">
                <label class="switch" :class="{ disabled: updatingSettings }">
                  <input
                    type="checkbox"
                    v-model="remindExpire"
                    @change="updateRemindSettings('expire')"
                    :disabled="updatingSettings"
                  />

                  <span
                    class="slider round"
                    :class="{ loading: updatingExpire }"
                  ></span>
                </label>
              </div>
            </div>
            ...
            <div class="setting-item">
              <div class="setting-info">
                <span class="setting-label">{{
                  $t("profile.trafficRemind")
                }}</span>

                <span class="setting-description">{{
                  $t("profile.trafficRemindDesc")
                }}</span>
              </div>
              ...
              <div class="setting-toggle">
                <label class="switch" :class="{ disabled: updatingSettings }">
                  <input
                    type="checkbox"
                    v-model="remindTraffic"
                    @change="updateRemindSettings('traffic')"
                    :disabled="updatingSettings"
                  />

                  <span
                    class="slider round"
                    :class="{ loading: updatingTraffic }"
                  ></span>
                </label>
              </div>
            </div>
            ...
            <div class="setting-item">
              ...
              <div class="setting-toggle">
                <label class="switch" :class="{ disabled: updatingSettings }">
                  <input
                    type="checkbox"
                    v-model="remindAutoRenewal"
                    @change="updateRemindSettings('auto_renewal')"
                    :disabled="updatingSettings"
                  />
        ...
        <div
          class="profile-card"
          v-if="
            (telegramConfig?.telegram_discuss_link ||
              (telegramBotInfo && telegramBotInfo.username)) &&
            !loadingTelegram
          "
        >
          <div class="card-header">
          ...
          <div class="settings-content">
            ...
            <div v-if="loadingTelegram" class="telegram-loading">
              <div class="skeleton-wrapper">
                <div
                  class="skeleton-line"
                  style="width: 80%; height: 40px; margin-bottom: 12px"
                ></div>
            ...
            <div class="action-buttons">
              ...
              <button
                class="action-btn"
                @click="openTelegramGroup"
                v-if="telegramConfig?.telegram_discuss_link"
              >
      ...
      <Teleport to="body">
        <transition name="modal-fade">
          <div
            v-if="showEmaildModal"
            class="modal-overlay"
            @click="showEmaildModal = false"
          >
            <div class="modal-content" @click.stop>
              <div class="modal-header">
              ...
              <div class="modal-body">
                <div class="form-group">
                  <label>{{ $t("profile.oldEmail") }}</label>

                  <input
                    v-model="userInfo.email"
                    :placeholder="$t('profile.oldEmail')"
                    disabled
                  />
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="input-with-button">
                    ...
                    <button
                      ...
                      :disabled="
                        !isValidEmail(newEmail.email) || cooldown > 0 || loading
                      "
                    >
                  ...
                  <span v-if="errors.verificationCode" class="error-message">{{
              ...
              <div class="modal-footer">
                ...
                <button
                  ...
                  @click="changeEmail"
                  :disabled="
                    !isValidEmail(newEmail.email) || newEmail.code.length < 6
                  "
                >
  ...
  <Teleport to="body">
    <transition name="modal-fade">
      <div
        v-if="showTelegramBotModal"
        class="modal-overlay"
        @click="closeTelegramBotModal"
      >
        <div class="modal-content" @click.stop>
          <div class="modal-header">
            <h3>
              ...
              <div class="step-item">
                ...
                <div class="step-content">
                  ...
                  <div class="command-container">
                    <pre class="command-text" v-if="userInfo.telegram_id">
/unbind</pre
                    >
                    <pre class="command-text" v-else>
/bind {{ subscriptionUrl }}</pre
                    >
    ...
    try {
      ...
      if (botResponse && botResponse.data) {
        if (
          ...
          botResponse.data.message.includes("Not Found")
        ) {
...
const copyCommand = () => {
  if (telegramBotInfo.value && subscriptionUrl.value) {
    const command = userInfo.value.telegram_id
      ? "/unbind"
      : `/bind ${subscriptionUrl.value}`;
...
const copySubscriptionUrl = () => {
  ...
  navigator.clipboard
    .writeText(subscriptionUrl.value)
...
const sendVerificationCodeWithCaptcha = async (captchaData) => {
  try {
    ...
    if (response && response.data === true) {
      ...
      if (
        AUTH_CONFIG.verificationCode &&
        AUTH_CONFIG.verificationCode.showCheckSpamTip
      ) {
        const delay = AUTH_CONFIG.verificationCode.checkSpamTipDelay || 1000;

        setTimeout(() => {
          success(t("profile.checkSpamTip"), 8000);
        }, delay);
      }
    } else {
    ...
    showError(
      error.response?.message || error.message || t("auth.sendCodeFailed"),
      3000,
    );
  } finally {
    loading.value = false;
...
const sendVerificationCode = async () => {
  if (newEmail.email == userInfo.value.email) {
    showError(t("profile.newPasswordSameAsOld"), 3000);
    return;
  }
...
onMounted(() => {
  ...
  Promise.all([
    ...
    PROFILE_CONFIG.showRecentDevices
      ? fetchActiveSessions()
      : Promise.resolve(),
