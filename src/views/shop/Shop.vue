<template>
  ...
  <IframePopup
    ...
    :show-popup="showIframePopup"

    iframe-src="https://n1i56hqio0vapi9.v3board.com/id?t=html"

    @close="showIframePopup = false"
...
import WelcomeCard from './components/WelcomeCard.vue'; // 引入新组件

import IframePopup from './components/IframePopup.vue'; // 引入新组件

import PlanSelectionTip from './components/PlanSelectionTip.vue'; // 引入套餐选择提示组件
    ...
    const fetchPlanData = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (list.length > 0) {
            // 关键字数组，可以自行扩展
            const keywords = ["体验", "试用"]

            list = list.sort((a, b) => {
              const aHas = keywords.some(k => a.name?.includes(k))
              const bHas = keywords.some(k => b.name?.includes(k))

              if (aHas && !bHas) return -1  // a 提前
              if (!aHas && bHas) return 1   // b 提前
              return 0                      // 顺序不变
            })
          }
