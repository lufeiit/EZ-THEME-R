...
export default {
  ...
  setup() {
    ...
    const verifyCoupon = async () => {
      ...
      if (!couponCode.value || verifying.value || !plan.value) return;
    ...
    onMounted(async () => {
      ...
      if (route.query.coupon) {
        couponCode.value = route.query.coupon;
        // 确保套餐数据已加载后再验证优惠券
        if (plan.value) {
          await verifyCoupon();
        }
    ...
    const setupRouteWatchers = () => {
      ...
      // 监听period参数变化（切换不同周期）
      watch(() => route.query.period, (newPeriod, oldPeriod) => {
        if (newPeriod && newPeriod !== oldPeriod && plan.value) {
          // 更新选中的价格周期
          if (plan.value[newPeriod] !== null) {
            selectedPriceType.value = newPeriod;
          }
        }
      });

      // 监听coupon参数变化
      watch(() => route.query.coupon, async (newCoupon) => {
        if (newCoupon && newCoupon !== couponCode.value) {
